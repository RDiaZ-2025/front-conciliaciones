...
• El formato debe ser MP4`
    ...
    constructor() {
        ...
        // Apply selection from SolutionSelectionDialog if present
        const selection = this.config.data?.selection;
        if (selection) {
            // 1. Set Category, Subcategory and Type
            this.form.get('solutionCategory')?.setValue(selection.category);

            if (selection.category === 'MOBILE') {
                this.form.get('solutionType')?.setValue(selection.solution);
            } else if (selection.category === 'PROGRAMMATIC') {
                this.form.get('solutionSubcategory')?.setValue(selection.type);
                this.form.get('solutionType')?.setValue(selection.solution);
            } else if (selection.category === 'CONTENT_RED_PLUS') {
                this.form.get('solutionSubcategory')?.setValue(selection.type);
                if (selection.type === 'CONTENIDO_RED_PLUS') {
                    this.form.get('solutionType')?.setValue('CONTENT_PUBLIRREPORTAJE');
                } else {
                    this.form.get('solutionType')?.setValue(selection.solution || 'CONTENT_PUBLIRREPORTAJE');
                }
            }

            this.selectedSolution.set(this.form.get('solutionType')?.value);
            this.updateValidators(this.selectedSolution() || '');

            // Disable selection fields
            this.form.get('solutionCategory')?.disable();
            this.form.get('solutionSubcategory')?.disable();
            this.form.get('solutionType')?.disable();
        }
    ...
    updateValidators(solution: string) {
        ...
        Object.keys(this.form.controls).forEach(key => {
            // Exclude structural fields that handle their own validation state
            if (key !== 'solutionType' && key !== 'solutionCategory' && key !== 'solutionSubcategory') {
                const control = this.form.get(key);
                control?.clearValidators();
                control?.setErrors(null); // Explicitly clear any errors
        ...
        if (solution === 'SMS') {
            this.setValidators('sms_clientName', [Validators.required, Validators.pattern(/^[A-Z0-9\s]+$/)]);
