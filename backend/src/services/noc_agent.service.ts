...
export class NocAgentService {
  private async getAdmanagerData(dias = 30) {
    ...
    const format = (d: Date | string | null | undefined) => d ? new Date(d).toISOString().split('T')[0] : '';
    ...
    const totalUsd = registros.reduce((acc, r) => acc + _safe(r.ingresosAdExchange), 0);
    const totalUsdAnt = registrosAnt.reduce((acc, r) => acc + _safe(r.ingresosAdExchange), 0);
    const totalImp = registros.reduce((acc, r) => acc + _safe(r.impresionesTotales), 0);
    const totalCop = registros.reduce((acc, r) => acc + _safe(r.cop), 0);

    const ecpmPromedio = totalImp > 0 ? (totalUsd / totalImp) * 1000 : 0;

    return {
      fuente: "AdManager (Portal Web)",
      dias_analizados: dias,
      fecha_inicio: format(inicio),
      fecha_fin: format(hoy),
      total_usd: Number(totalUsd.toFixed(2)),
      total_cop: Number(totalCop.toFixed(2)),
      total_impresiones: totalImp,
      ecpm_promedio: Number(ecpmPromedio.toFixed(2)),
      variacion_vs_periodo_anterior: _variacion(totalUsd, totalUsdAnt),
      ultimo_registro: registros.length && registros[0].fecha ? format(registros[0].fecha) : 'Sin registros',
      detalle_reciente: registros.slice(0, 5).map(r => ({
        fecha: format(r.fecha),
        usd: _safe(r.ingresosAdExchange),
  ...
  private async getYoutubeData(meses = 3) {
    ...
    let topCanalUsd = -1;
    if (filas.length > 0) {
      for (const [canal, val] of Object.entries(filas[0].canales)) {
        if (val > topCanalUsd) {
          topCanalUsd = val;
          topCanal = canal;
        }
    ...
    return {
      ...
      mejor_canal_ultimo_mes: { canal: topCanal, usd: topCanalUsd },
      detalle_por_mes: filas
    };
  }

  private async getFacebookData(meses = 3) {
    const repo = AppDataSource.getRepository(IngresoRedes);
    const hoy = new Date();
    const inicio = new Date(hoy.getFullYear(), hoy.getMonth() - meses, 1);
    const format = (d: Date) => d.toISOString().split('T')[0];
  ...
  private async getPresupuestoData(meses = 3) {
  ...
  async processChat(message: string, history?: any[]) {
    ...
    const [admanager, youtube, facebook, presupuesto] = await Promise.all([
      this.getAdmanagerData(30),
      this.getYoutubeData(3),
      this.getFacebookData(3),
      this.getPresupuestoData(3)
    ]);
...
export const nocAgentService = new NocAgentService();
