...
export class NocDashboardService {
  private applyFilters(qb: SelectQueryBuilder<DashboardData>, filters: DashboardFilterQuery): void {
    const { start_date, end_date, source, section, author, topic, category } = filters;
  ...
  async getOverviewStats(filters: DashboardFilterQuery) {
    this.ensureDatabase();
    ...
    const cards = {
      ...
      active_source: (filters.source as string) || "Todas"
    };
    ...
    this.applyFilters(volHistQb, filters);
    ...
    this.applyFilters(trendQb, filters);
  ...
  async getContentStats(filters: DashboardFilterQuery) {
    this.ensureDatabase();
    ...
    const topicQb = AppDataSource.getRepository(DashboardData).createQueryBuilder('d')
      ...
      .addSelect('SUM(d.sessions)', 'sessions')
      .addSelect('SUM(d.engagedSessions)', 'engaged_sessions');
    this.applyFilters(topicQb, filters);
    ...
    const authorQb = AppDataSource.getRepository(DashboardData).createQueryBuilder('d')
      ...
      .addSelect('SUM(d.engagedSessions)', 'engaged_sessions')
      .addSelect('COUNT(d.id)', 'articles');
    this.applyFilters(authorQb, filters);
  ...
  async getEntitiesStats(filters: DashboardFilterQuery) {
    this.ensureDatabase();
    const { end_date } = filters;
    ...
    this.applyFilters(entityQb, filters);
    ...
    this.applyFilters(comboQb, filters);
  ...
  async getEntityDetail(entity: string, filters: DashboardFilterQuery) {
    this.ensureDatabase();

    if (!entity) {
      throw new Error('Parámetro entity es requerido');
    }
    ...
    this.applyFilters(articleIdsQb, filters);
  ...
  async getReachStats(filters: DashboardFilterQuery) {
    this.ensureDatabase();
    ...
    this.applyFilters(usersQb, filters);
    ...
    const viewsQb = AppDataSource.getRepository(DashboardData).createQueryBuilder('d')
      .select('d.temaPrincipal', 'topic')
      .addSelect('SUM(d.screenPageViews)', 'views');
    this.applyFilters(viewsQb, filters);
    ...
    const depthQb = AppDataSource.getRepository(DashboardData).createQueryBuilder('d')
      ...
      .addSelect('SUM(d.screenPageViews)', 'views')
      .addSelect('SUM(d.sessions)', 'sessions');
    this.applyFilters(depthQb, filters);
  ...
  async getAudienceStats(filters: DashboardFilterQuery) {
    this.ensureDatabase();
    ...
    const fidelityQb = AppDataSource.getRepository(DashboardData).createQueryBuilder('d')
      ...
      .addSelect('SUM(d.totalUsers)', 'users')
      .addSelect('SUM(d.engagedSessions)', 'engaged_sessions');
    this.applyFilters(fidelityQb, filters);
  ...
  async getDashboardFilters() {
    ...
    const [authors, topics, categories, sections, sources] = await Promise.all([
      getDistinct('autor'),
      getDistinct('temaPrincipal'),
      getDistinct('categoriaEntidad'),
      getDistinct('seccion'),
      getDistinct('fuente')
    ]);
...
export const nocDashboardService = new NocDashboardService();
