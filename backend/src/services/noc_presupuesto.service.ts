...
export class NocPresupuestoService {
  ...
  async getDashboardPresupuesto(year = 2026, filter_type = 'TOTAL') {
    this.ensureDatabase();
  ...
  async importarPresupuesto() {
    ...
    if (!foundPath) {
      throw new Error('Archivo ppto_2026.xlsx no encontrado.');
    }
    ...
    if (!hasFecha) {
      throw new Error('El archivo Excel no tiene un formato válido o falta la columna "Fecha".');
    }
...
export const nocPresupuestoService = new NocPresupuestoService();
